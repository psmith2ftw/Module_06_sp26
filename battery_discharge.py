...
WHAT IS PROVIDED:
  ...
  - Depth-of-discharge (DoD) x-axis plot showing three current levels
...
WHAT YOU FILL IN  (search for  ← YOUR WORK):
  ...
  4. The three discharge currents
  5. V_device_threshold — your device's minimum operating voltage
...
#   This is what makes discharge rate matter.
...
R_int = 20.0    # ← YOUR WORK: choose a value appropriate for your cell type (Ω)
...
# Simulation stops when V_terminal drops below this value.
# For the IoT / coin cell application: set to your device's minimum supply voltage.
# If you don't have a specific device: use 0.70 * E_std.
V_device_threshold = 3.00     # ← YOUR WORK (V)

# ─────────────────────────────────────────────────────────────────────────────
# DISCHARGE CURRENTS  ← YOUR WORK
# For the coin cell / IoT application, show:
#   design current | 10× overload | 100× overload
# ─────────────────────────────────────────────────────────────────────────────

currents_mA = {
    'Design    (0.10 mA)':   0.10,
    'Moderate   (1.0 mA)':   1.00,
    'Burst     (10.0 mA)':  10.00,
}
...
def run_discharge(I_mA):
    ...
    Returns arrays of depth-of-discharge, open-circuit voltage,
    terminal voltage, and time, plus scalar summary metrics.
    """
    I_A = I_mA / 1000.0
    ...
    usable_mAh = I_mA * t_h_arr[-1]                    # actually delivered
    eff_pct    = 100.0 * dod_arr[-1]                   # reactant fraction consumed

    return dict(dod=dod_arr, V_oc=V_oc_arr, V_term=V_term_arr,
                t_h=t_h_arr, usable_mAh=usable_mAh,
...
if E_std == 0.0 or n == 0:
    ...
    ax1.set_title('Terminal Voltage vs. Depth of Discharge\n'
    ...
    print(f"  {'Current':22s}  {'Usable (mAh)':>14}  {'DoD at cutoff':>15}")
    print(f"  {'─'*55}")
    for label, res in results.items():
        print(f"  {label:22s}  {res['usable_mAh']:>14.4f}  "
              f"{res['eff_pct']:>13.1f}%")
